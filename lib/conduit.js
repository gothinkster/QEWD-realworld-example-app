...
var conduit_ws = require('./conduit-ws');
...
module.exports = {
  ...
  beforeHandler: function(messageObj, session, send, finished) {
    ...
    if (messageObj.type === 'ewd-qoper8-express') return;

    // before-message checks for WebSocket (ie non-REST) messages

    var authenticationOptional = [
      'test',
      'tags.get',
      'users.login',
      'users.register',
    ...
    if (!authenticationOptional.includes(messageObj.type)) {
      if (!session.authenticated) {
        var errors = errorHandler.add('user', "You must be logged in");
        errorHandler.errorResponse(errors, finished);
        return false;
      }
    }
  },
  ...
  init: function(application) {
    ...
    conduit_ws.init.call(this, application);
